...
class Xor(ResilientSurfaceClient):
    def __init__(self):
        super().__init__(
            stream_client=StreamClient(),
            bootstrap_runtime=PhiRuntime(LIB_ROOT),
...
#         """가벼운 핑(Ping) 후 실패 시 런타임 본연의 ensure() 호출"""
#         try:
#             req = urllib.request.Request(self.fallback_url, method="HEAD")
#             urllib.request.urlopen(req, timeout=1.0)
#         except Exception:
#             log.warning("[xor] 표면 응답 없음. XphiRuntime 부트스트랩을 호출합니다.")
#             try:
#                 # 복잡한 예외 처리나 subprocess 직접 호출 없이 깔끔하게 위임
#                 self.bootstrap_runtime.ensure()
#             except Exception as e:
#                 log.error(f"[xor] 런타임 복구 실패: {e}")
