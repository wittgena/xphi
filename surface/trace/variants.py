...
import unittest
import time
from bridge.plane.emitter import get_emitter, flow_scope, _flow_context
from bridge.plane.bound import BoundPlane

class ToposVerification(unittest.TestCase):
    def setUp(self):
        """
        ...
        - flow context reset
        """
        BoundPlane.meter.history.clear()
        BoundPlane.fold_cache.clear()
        _flow_context.set({})
    ...
    def test_laminar_flow(self):
        ...
        self.assertEqual(len(BoundPlane.fold_cache), 0, "Laminar flow should not trigger folding.")
        ...
        self.assertIn(key1, BoundPlane.meter.history)
        self.assertIn(key2, BoundPlane.meter.history)
    ...
    def test_turbulence_and_folding(self):
        ...
        self.assertIn(key, BoundPlane.fold_cache, "High density events must be caught in fold_cache.")
        
        folded_event = BoundPlane.fold_cache[key]
    ...
    def test_attenuation_and_implicit_flush(self):
        ...
        self.assertIn(key, BoundPlane.fold_cache)
        
        ## simulate pressure relaxation
        time.sleep(BoundPlane.meter.window + 0.1)
        ...
        self.assertNotIn(key, BoundPlane.fold_cache, "Fold cache must be flushed implicitly after pressure drops.")
    ...
    def test_explicit_flush(self):
        ...
        self.assertEqual(len(BoundPlane.fold_cache), 0, "Explicit flush must clear all folded events.")
