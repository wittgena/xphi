...
class PriceTrajectory:
    ...
    def __init__(self, symbol: str, points: List[PricePoint]):
        self.symbol = symbol
        self.points = points
...
class StructureAggregator:
    ...
    @staticmethod
    def compose(name: str, trajectories: List[PriceTrajectory]) -> PriceTrajectory:
        if not trajectories:
            return PriceTrajectory(name, [])
        ...
        points = [PricePoint(timestamp=ts, price=float(val)) for ts, val in avg_series.items()]
        return PriceTrajectory(name, points)

class MarketField:
    @staticmethod
    def load(symbol: str, start: str, end: str) -> PriceTrajectory:
        df = fdr.DataReader(symbol, start, end)
        points = [PricePoint(pd.to_datetime(ts), float(price)) for ts, price in df["Close"].items()]
        return PriceTrajectory(symbol, points)

class ReturnWindowStrategy:
    def __init__(self, window_days: int):
        self.window_days = window_days

    def generate(self, trajectory: PriceTrajectory) -> List[WindowedReturn]:
