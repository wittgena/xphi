...
class AbsorbCluster:
    ...
    def run(self, apply: bool = False):
        ...
        print(f"## SEMANTIC CLUSTER ABSORBER (Mode: {'APPLY' if apply else 'DRY-RUN'})")
        print("="*65)
        print(f"Source (dissolve_target) : {len(all_src_files)} files")
        print(f"Dest (meta)              : {dest_count} files")
