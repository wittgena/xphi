...
async def main():
    ## 모듈 디스커버리 (watcher.kernel.exchange 등이 자동 스캔되어 'exahange' 커널 등록됨)
    discover_modules(find_current_self())
    log = get_emitter("rhythm.exchange", phase="BOOT")
    
    ## ExchangeSensor(시장 평균장)에 맞춘 JSON Payload
    redis_payload = """
    {
      "system_type": "MARKET_MEAN_FIELD_ATTRACTOR",
