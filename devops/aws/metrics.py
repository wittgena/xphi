...
class AWSEnvironment:
    ...
    def emit_snapshot(self) -> AWSMetricsSnapshot:
        """
        @phase.emit:
...
class Node:
    ...
    def ingest(self, metrics: AWSMetricsSnapshot):
        """@phase.map: Ψ → ∂Φ"""
...
class AWSEnvSimCoordinator:
    ...
    def emit_metrics(self) -> AWSMetricsSnapshot:
        return AWSMetricsSnapshot(
            timestamp=time.time(),
            resource_id=self.state["resource_id"],
...
if __name__ == "__main__":
    sim = AWSEnvSimCoordinator(size=10)
    asyncio.run(sim.run())