...
from kube.validator.compose import KubeFlowValidator, KubeSelfExtractor 
...
class TranscriptEngine:
    def __init__(self):
        try:
            ...
            # 캐시 경로 정의 (필요시 resolve_path 활용)
            self.outlet_root = resolve_path("k8s")
        except Exception:
            self.self_root = Path.cwd()
            self.outlet_root = Path.cwd() / ".anchor" / "cache" / "k8s"
        self.ensure_env()
...
def run_pipeline(md_path: str):
    ...
    surface = KubeSelfExtractor.extract(blocks)
