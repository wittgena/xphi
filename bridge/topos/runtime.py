...
class BaseNode:
    ...
    async def run(self, flow: ToposFlow, operator: Any, ctx: FlowState) -> List[Tuple[str, FlowState]]:
        pass
...
@proto(Proto((ToposFlow, ToposOperator, "State"), kind="topos"))
class ToposNode(BaseNode):
    ...
    async def run(self, flow: ToposFlow, operator: ToposOperator, ctx: FlowState) -> List[Tuple[str, FlowState]]:
        log.info(f"  [ToposNode] Processing state -> Name: {self.name}, Kind: {self.kind}")
        return [(self.next, ctx)]

@proto(Proto((ToposFlow, Inversion, "State"), kind="linker"))
class LinkerNode(BaseNode):
    def __init__(self, spec: dict, pool: Any = None, **kwargs):
        super().__init__(spec, pool, **kwargs)
        self.next = spec.get("next", "END")

    async def run(self, flow: ToposFlow, operator: Inversion, ctx: FlowState) -> List[tuple]:
...
@proto(Proto((ToposFlow, Inversion, "State"), kind="inversion"))
