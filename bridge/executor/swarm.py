...
from bridge.executor.cli import _GenericCliExecutor
...
class SwarmCliExecutor(BaseExecutor):
    ...
    async def execute(self, psi) -> list:
        ...
        with flow_scope(flow_id=task_id, phase="EXECUTION"):
            ...
            try:
                module = importlib.import_module(task_info_list[0]["module_fqn"])
                if hasattr(module, "entry_task"):
                    ## 동일한 cli_args를 주입하여 동일한 Orchestrator 인스턴스를 생성
                    task_adapter = module.entry_task(cli_args)
