...
from bridge.plane.bound import default_plane
...
class PhaseEmitter:
    ...
    def signal(self, msg, *args, **kwargs): 
        """PhaseEmitter 고유 확장 메서드"""
        self._log("SIGNAL", msg, *args, **kwargs)

    def flush(self):
        default_plane.flush()

def get_emitter(name: str, phase: Optional[str] = None, boundary: Optional[str] = None, mode: str = "NORMAL") -> PhaseEmitter:
    return PhaseEmitter(name, phase, boundary, mode=mode)

def get_logger(name: str, phase: Optional[str] = None, boundary: Optional[str] = None, mode: str = "NORMAL") -> PhaseEmitter:
    """@legacy_adapter"""
    return get_emitter(name, phase, boundary, mode=mode)
