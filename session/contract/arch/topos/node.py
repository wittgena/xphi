# session.contract.arch.topos.node
import asyncio
import enum 
import logging
from typing import List, Tuple, Any, Optional, Dict
from meta.flow.surface.emitter import get_emitter
from session.contract.proto.col import proto, get_proto, Proto
from session.contract.proto.flow import ProtoFlow, FlowState
from session.contract.arch.topos.trans import TransRule, PhaseSpec, NodeType
...
class PhaseOperator:
    """PhaseNode 구동을 위한 더미 오퍼레이터 (필요 시 로직 확장)"""
    pass

class Inversion:
    def mutate(self, spec: PhaseSpec, root: 'PhaseNode') -> List[Dict]:
        residues = []
        ...
        for rule in spec.rules:
            ...
            if rule.action == "INVERT":
                ...
                # [수정됨] PhaseNode가 BaseNode 스펙을 따르도록 spec dict 전달
...
@proto(Proto((ProtoFlow, PhaseOperator, "State"), kind="phase"))
class PhaseNode(BaseNode):
    def __init__(self, spec: dict, pool: Any = None, **kwargs):
        ...
        self.children: Dict[str, 'PhaseNode'] = spec.get("children", {})
        self.next = spec.get("next", "END")

    async def run(self, flow: ProtoFlow, operator: PhaseOperator, ctx: FlowState) -> List[Tuple[str, FlowState]]:
        log.info(f"  [PhaseNode] Processing state -> Name: {self.name}, Kind: {self.kind}")
