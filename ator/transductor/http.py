...
@ator_contract("http.post.transductor")
class HttpPostTransductor(HttpBaseTransductor):
    """@flow: 대상 시스템에 자극(Ψ)을 주입하여 상태 전이를 유도"""
    def _project(self, flow: FlowFlow, ator_node: Any) -> dict:
...
@ator_contract("http.get.transductor")
class HttpGetTransductor(HttpBaseTransductor):
