# xphi.xor.parser.env
## @lineage: xphi.arch.xor.parser.env
import importlib
import inspect
import json
import os
...
from xphi.arch.model.surge.disc import DiscMixin, get_known_concrete_subclasses
from xphi.arch.event.next import ToposId
...
def get_env_parser(target_type: type, parsers: dict[type, EnvParser]) -> EnvParser:
    ...
    if origin and issubclass(origin, BaseModel):
        target_type = origin
    if issubclass(target_type, DiscMixin) and (
