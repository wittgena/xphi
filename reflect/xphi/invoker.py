...
from bridge.interface.topos import IEventBus, IPhaseField, IPhaseAtor
...
XPHI_API_BASE = os.getenv("XPHI_API_BASE", "http://localhost:8080/judgment")
...
class XphiProjector(ResilientSurfaceClient, IPhaseAtor):
    """
    @role: Φ(t) Projector -> Xphi 연동 액터
    """
    def __init__(self, ator_id="xphi.projector"):
        super().__init__(
            stream_client=StreamClient(),
            bootstrap_runtime=XphiRuntime(LIB_ROOT),
            redis_surface=RedisSurface(REDIS_HOST, REDIS_PORT),
    ...
    async def react(self, event: PsiEvent, bus: IEventBus):
        ...
        await asyncio.to_thread(self._invoke_xphi, event)

    def _invoke_xphi(self, event: PsiEvent):
