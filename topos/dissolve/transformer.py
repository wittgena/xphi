...
from topos.dissolve.model import DissolveModel, NodeData, EdgeData, LoopEdgeData, ToposGraphSchema
...
class ToposAnalyzer:
    ...
    def get_dissolve_schema(self, module_index: Dict[str, Path], root: Path) -> ToposGraphSchema:
        ...
        return ToposGraphSchema(meta=meta, nodes=nodes, topos_edges=edges, loop_edges=loop_edges, cycles=cycles)
...
class BoundManager:
    ...
    def _write_json(self, data: ToposGraphSchema):
...
def main():
    ...
    parser.add_argument("--dir", required=True, help="Target python project directory")
    args = parser.parse_args()

    manager = BoundManager(args.dir)
    schema = manager.run()
