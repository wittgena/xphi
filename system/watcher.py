...
from system.gitops.emitter import GitOpsEmitter
from anchor.resolver import find_current_self

log = get_emitter("system.watcher", phase="BOOT")

@watcher_contract("system.watcher")
class SystemWatcher(ICriticalDetector):
    """
    @role: ∂Φ 임계 감시자
        ...
        for node_id, data in field.get_state().items():
            ...
            if carrier:
                ...
                return PsiEvent(
                    ...
                    source_id="system.watcher", scope="SYSTEMIC", tick=current_tick, carrier=carrier
                )
        return None
...
async def main():
    """@entry: System Watcher System Boot Sequence"""
