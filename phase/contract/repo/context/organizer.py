# phase.contract.repo.context.organizer
import os
import sys
import json
import argparse
...
from phase.contract.repo.context.graph import ContextGraph, SurfaceTemplateData, _extract_rel_attr, EntryTemplate
from bound.resonance.schema import ResonanceGraph, ResonanceNode
from meta.project.lang.binder import LangBinder
...
log = get_logger("context.organizer")

class ContextSelector:
    @staticmethod
    def select(graph_data: dict, context: ContextGraph) -> ResonanceGraph:
...
class ContextProjector:
    ...
    @classmethod
    def translate(cls, subgraph: ResonanceGraph, context: ContextGraph) -> SurfaceTemplateData:
...
class BoundRenderer:
    ...
    def project_file(self, graph_file: Path, context: ContextGraph) -> str:
...
@cli_contract(name="context.organizer", recept=["lang.binder"])
def main():
    bound_args, remain = parse_local(sys.argv[1:])
    if bound_args.local:
        entry_task(remain).run()
    else:
        dispatch_cli("context.organizer", entry_task, __file__)
