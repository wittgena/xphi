...
class WasmTester:
    ...
    async def _await_rupture(self) -> None:
        """@desc: 테스트가 도는 동안 백그라운드에서 붕괴(Collapse) 시그널을 폴링합니다."""
    ...
    # [수정] async def로 변경하여 하위 시나리오의 비동기 실행(await)을 지원
    async def _run_all_suites(self, broker: WasmBroker) -> int:
        """
        @desc: 비동기 컨텍스트 내에서 선택된 시나리오 스위트만 순차적으로 실행합니다.
        """
        total_fails = 0
        ...
        if run_all or "sandbox" in self.suites:
            ...
            await sandbox_suite.run_all() # [수정] await 추가
        ...
        if run_all or "ledger" in self.suites:
            ...
            await ledger_suite.run_all() # [수정] await 추가
        ...
        if run_all or "a2a" in self.suites:
            ...
            await a2a_suite.run_all() # [수정] await 추가
        ...
        if run_all or "ecosystem" in self.suites:
            ...
            await ecosystem_suite.run_all() # [수정] await 추가
            total_fails += ecosystem_suite.fail_count
            
        return total_fails

    async def execute(self) -> Tuple[bool, str]:
        """테스트 샌드박스를 구축하고 시나리오를 비동기로 실행합니다."""
        ...
        try:
            ...
            # [추가] 유령 데몬 간섭을 원천 차단하기 위해 테스트 전용 스트림 격리
            ...
            # [수정] 스레드 위임(asyncio.to_thread)을 제거하고, 순수 비동기로 직접 태스크 생성
