...
from model.surface.entry import GraphContext, SurfaceTemplateData, _extract_rel_attr, EntryTemplate
from phase.model import PhaseGraph, PhaseNode
from receptor.lang.binder import LangBinder
from contract.registry import cli_contract
from contract.executor.cli import dispatch_cli
from contract.executor.cli import execute_cli_task, CliTaskAdapter, parse_local

log = get_logger("model.entry")

class EntrySelector:
    @staticmethod
    def select(graph_data: dict, context: GraphContext) -> PhaseGraph:
        full_nodes = {k: PhaseNode(**v) for k, v in graph_data.get("nodes", {}).items()}
        
        entry_id = context.entry
        if entry_id not in full_nodes:
            log.warning(f"Entry point '{entry_id}' not found in the graph.")
            return PhaseGraph(invariants=[], nodes={})
        ...
        for _ in range(context.depth):
            ...
            for n_id in current_level:
                for rel in full_nodes[n_id].relations:
                    ...
                    if rel_type in context.valid_relations and rel_target in full_nodes:
        ...
        for n_id in selected_ids:
            ...
            for r in node.relations:
                ...
                if r_target in selected_ids and r_type in context.valid_relations:
                    filtered_rels.append(r)
            
            sub_nodes[n_id] = PhaseNode(
        ...
        return PhaseGraph(invariants=[], nodes=sub_nodes)

class EntryProjector:
    @staticmethod
    def _get_total_strength(node: PhaseNode) -> int:
        """노드별 전체 관계 강도 합산"""
        return sum(_extract_rel_attr(r, "strength", 1) for r in node.relations)

    @classmethod
    def translate(cls, subgraph: PhaseGraph, context: GraphContext) -> SurfaceTemplateData:
...
class ContextProjector:
    ...
    def project(self, compiled_graph: dict) -> str:
        ...
        surface_data = EntryProjector.translate(subgraph, self.context)
...
class EntryOrganizer:
    ...
    def loop(self):
        if not self.fixed_graph_path.exists() or self.rebuild:
            log.info("[Auto-Bind] Graph topology missing or rebuild requested. Orchestrating ModelBinder...")
            binder = LangBinder()
            if self.target_dir:
...
def entry_task(args):
    ...
    return CliTaskAdapter(orchestrator.loop)

@cli_contract(name="model.entry", recept=["model.binder"])
def main():
    bound_args, remain = parse_local(sys.argv[1:])
    if bound_args.local:
        entry_task(remain).run()
    else:
        dispatch_cli("model.entry", entry_task, __file__)
